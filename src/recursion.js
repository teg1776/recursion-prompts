...
var arraySum = function(array) {
	if (array.length === 0) {
		...
		let flattened = array.toString().split(",").map(Number); 
		return flattened[0] + arraySum(flattened.slice(1));
	}
...
var range = function(x, y) {
 	...
 	if (x < y){
 		return [x + 1].concat(range(x+1, y)); // => [3] + ( range(3, 9) => [4] ) === [3, 4] + ( range(4, 9) => [5] ) === [3, 4, 5]...
 	}
 	if (x > y){
...
var fizzBuzz = function(n) {
	...
	return fizzBuzz(n-1).concat([placeHolder]);
};
...
var capitalizeFirst = function(array) {
	...
	var str = array[0]; // string inside to a string
	str = str.charAt(0).toUpperCase() + str.slice(1); // cap first letter
...
var augmentElements = function(array, aug) {
	...
	result.push([].concat(array[0], aug))
